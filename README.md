...
En tant qu’ **Ingénieur de données** dans une société de conseil en analytique, j'ai été assigné à un projet stratégique visant à **décongestionner les autoroutes nationales**. L’objectif est d’analyser les données de trafic routier collectées à partir de différents postes de péage. Chaque autoroute est exploitée par un opérateur distinct, avec une configuration informatique propre et des formats de fichiers hétérogènes (CSV, TSV, Fixed-Width).  
...
[🎬 Voir la vidéo de démonstration](./ETL-AIRFLOW.mp4)
